...
PulseStudio v0.2.126 is connected to the PulseStudio PostHog Cloud project (US region).
...
PulseStudio v0.2.126 retains the broad privacy-limited instrumentation for product usage, trends, reliability, and errors, with the corrected PostHog project token. Event properties use control IDs, categorical settings, counters, duration buckets, and sanitized error names/codes rather than user content.
...
- Bookmarks saved
- My Voice highlight and voice-profile events are intentionally inactive in v0.2.126 while the feature remains disabled for audio-isolation testing
